...
class EyeAnimation(util.Component):
    ...
    def _image_to_data(self, bgr_image):
    ...
    def _block_display(self, robot, image, x, y):
        ...
        """
        h, w = image.shape[:2]
        print(f"Display area {x+w-1}, {y+h-1} must not exceed dimensions of screen {w}x{h}")
        assert self._in_range((x, y), (x+w-1, y+h-1)), f"Display area {x+w-1}, {y+h-1} must not exceed dimensions of screen {w}x{h}"
        x, y = y, 240-x-w
        robot.display(self._image_to_data(np.rot90(image)), x, y, x+h-1, y+w-1)
    ...
    async def animate(self, robot, start_exp=None):
        self._color = util.bgr('orange')#util.bgr(self._robot.env.vars.get('eye_color', 'cyan'))
        ...
        while True:
            ...
            if blink:
                ...
                self._block_display(robot, self._canvas[oy0: yt, ox0: ox1], ox0, oy0)
                oy0 = yt
            ...
            self._block_display(robot, self._canvas[by:by+bh, bx:bx+bw], bx, by)
